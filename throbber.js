(function () {
    'use strict';

    var $ = angular.element;
    var throbberDefault = '<img width="64" height="64" style="position: absolute; top: 50%; left: 50%; margin-left: -32px; margin-top: -32px;" src="data:image/gif;base64," />';
    var styleTemplate = 'style="width: ${width}px; height: ${height}px; top: ${top}px; left: ${left}px;"';
    var throbberTemplate = '<div class="${classes}" ${style}></div>';
    var backdropTemplate = '<div class="throbber-backdrop" ${style}></div>';
    var numofConnections = 0;
    var removeTargets = [];
    var buildElements = function (parent, $window) {
        if (parent === $window) {
            var documentElement = document.documentElement;
            var backdropStyle = styleTemplate
                    .replace('${width}', documentElement.offsetWidth)
                    .replace('${height}', documentElement.offsetHeight)
                    .replace('${top}', documentElement.offsetTop)
                    .replace('${left}', documentElement.offsetLeft);
            var baseStyle = styleTemplate
                    .replace('${width}', parent.innerWidth)
                    .replace('${height}', parent.innerHeight)
                    .replace('${top}', 0)
                    .replace('${left}', 0);
            var throbberHtml = throbberTemplate
                .replace('${classes}', 'throbber-base-fixed')
                .replace('${style}', baseStyle);
            return {
                backdrop: $(backdropTemplate.replace('${style}', backdropStyle)),
                base: $(throbberHtml)
            };
        }

        var style = styleTemplate
            .replace('${width}', parent.offsetWidth)
            .replace('${height}', parent.offsetHeight)
            .replace('${top}', parent.offsetTop)
            .replace('${left}', parent.offsetLeft);
        var throbberHtml = throbberTemplate
            .replace('${classes}', 'throbber-base-absolute')
            .replace('${style}', style);
        return {
            backdrop: $(backdropTemplate.replace('${style}', style)),
            base: $(throbberHtml)
        };
    };
    var show = function (throbber, $window) {
        var parent = throbber.parent;
        var elements = buildElements(parent, $window);
        var body = $(document.querySelector('body'));

        if (typeof(throbber.throbber) === 'string') {
            elements.base.html(throbber.throbber);
        } else {
            elements.base.append(throbber.throbber);
        }

        body.append(elements.backdrop);
        body.append(elements.base);
        removeTargets = elements;
    };
    var hide = function () {
        angular.forEach(removeTargets, function (target) {
            target.remove();
        });
        removeTargets = [];
    };
    var normalizeConfig = function (config, $window) {
        var throbberConfig;
        if ('$throbber' in config) {
            throbberConfig = config.$throbber;
            delete config.$throbber;

            if (!('parent' in throbberConfig)) {
                throbberConfig.parent = $window;
            }
            if (!('throbber' in throbberConfig)) {
                throbberConfig.throbber = throbberDefault;
            }
        } else {
            throbberConfig = {
                parent: $window,
                throbber: throbberDefault
            };
        }
        var parent = throbberConfig.parent;
        if (typeof(parent) === 'string') {
            parent = document.querySelector(parent);
            if (!parent) {
                parent = $window;
            }
            throbberConfig.parent = parent;
        }
        return throbberConfig;
    };
    var interceptor = function ($q, $window) {
        return {
            request: function (config) {
                var throbberConfig = normalizeConfig(config, $window);
                if (numofConnections++ === 0) {
                    show(throbberConfig, $window);
                }
                return config;
            },
            response: function (response) {
                if (--numofConnections === 0) {
                    hide();
                }
                return response;
            },
            responseError: function (rejection) {
                if (--numofConnections === 0) {
                    hide();
                }
                return $q.reject(rejection);
            }
        };
    };

    var module = angular.module('interceptors', []);
    module.factory('throbberInterceptor', ['$q', '$window', interceptor]);
})();
